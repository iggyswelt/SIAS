...
# Quelle: agent_tasks Tabelle — live aus der metamaus DB

@app.route('/api/db/tasks', methods=['GET'])
def get_agent_tasks():
    """Workflow-Ansicht: Alle agent_tasks live aus der DB"""
    try:
        conn = get_db_connection()
        cursor = conn.cursor(cursor_factory=RealDictCursor)
        ...
        cursor.execute(f"""
            SELECT id, agent, task, priority, status,
                   created_at, started_at, done_at, result, created_by
            FROM agent_tasks
            {where_sql}
            ORDER BY
                CASE status
...
def create_agent_task():
    """Neuen Task in agent_tasks einwerfen"""
    try:
        data = request.get_json()
        ...
        cursor = conn.cursor()
        cursor.execute("""
            INSERT INTO agent_tasks (agent, task, priority, created_by)
            VALUES (%s, %s, %s, %s)
            RETURNING id
...
def stop_agent_task(task_id):
    ...
    try:
        ...
        cursor.execute("""
            UPDATE agent_tasks
            SET status = 'stopped', done_at = NOW(),
                result = COALESCE(result, '') || ' [manuell gestoppt]'
...
import requests
# yfinance ENTFERNT — blockierte Flask-Threads ohne Timeout!
# Alle Preise jetzt via REST APIs mit kurzen Timeouts.

import threading

def _load_cached_prices():
    """Load last known prices from cache file."""
    try:
...
def _save_cached_prices(data):
    """Cache current prices for next request fallback."""
    try:
        import json as _json
...
def _fetch_prices_background():
    """Background thread: fetch ALL prices and cache them."""
    ...
    # GOLD, SILVER via metals.live (free, no API key, fast)
    try:
        r = requests.get(
        ...
        if isinstance(r, list):
            for item in r:
                if item.get('metal') == 'gold' and item.get('price'):
                    prices['gold'] = float(item['price'])
                elif item.get('metal') == 'silver' and item.get('price'):
                    prices['silver'] = float(item['price'])
    except:
        pass

    # DAX, SP500 via CoinGecko indices (free, no auth)
    try:
        r = requests.get(
            "https://api.coingecko.com/api/v3/indices",
            timeout=5).json()
        if r.get('indices'):
            for idx in r['indices']:
                if 'german' in idx.get('name', '').lower() and idx.get('data_point'):
                    prices['dax'] = float(idx['data_point'])
                elif 's&p' in idx.get('name', '').lower() and idx.get('data_point'):
                    prices['sp500'] = float(idx['data_point'])
    except:
        # Fallback: Yahoo Finance quotes (simpler than yfinance.download)
        try:
            r = requests.get(
                "https://query1.finance.yahoo.com/v8/finance/chart/GC=F?range=7d&interval=1d",
                timeout=5).json()
            if r.get('chart', {}).get('result'):
                ...
                if closes:
                    ...
                    if len(closes) >= 2:
                        prev = closes[-2]
                        changes['gold_24h'] = ((closes[-1] - prev) / prev * 100) if prev > 0 else 0
        except:
            pass
        try:
            r = requests.get(
                "https://query1.finance.yahoo.com/v8/finance/chart/SLV?range=7d&interval=1d",
                timeout=5).json()
            if r.get('chart', {}).get('result'):
                result = r['chart']['result'][0]
                closes = result.get('indicators', {}).get('quote', [{}])[0].get('close', [])
                closes = [c for c in closes if c]
                if closes:
                    prices['silver'] = closes[-1]
                    if len(closes) >= 2:
                        prev = closes[-2]
                        changes['silver_24h'] = ((closes[-1] - prev) / prev * 100) if prev > 0 else 0
        except:
            pass
        try:
            r = requests.get(
                "https://query1.finance.yahoo.com/v8/finance/chart/^GDAXI?range=1d&interval=5m",
                timeout=5).json()
            if r.get('chart', {}).get('result'):
                result = r['chart']['result'][0]
                closes = result.get('indicators', {}).get('quote', [{}])[0].get('close', [])
...
# Global: last known prices (served instantly, updated in background)
...
def get_prices():
    ...
    # Serve cached data if fresh enough (< 50s)
    if _last_price_result and (now - _last_price_time) < 50:
        return jsonify(_last_price_result)
    
    # Serve file cache as fallback if we have stale data
    ...
    # Absolute fallback
    return jsonify({
        'btc': 'N/A', 'sol': 'N/A', 'ada': 'N/A',
        'gold': 'N/A', 'silver': 'N/A', 'dax': 'N/A', 'sp500': 'N/A',
        'btc_24h': 0, 'sol_24h': 0, 'gold_24h': 0, 'gold_7d': 0,
        'silver_24h': 0, 'silver_7d': 0,
        'created_at': datetime.now(pytz.timezone('Europe/Berlin')).isoformat()
    })
...
def agents_list():
    """Dynamische Agent-Liste aus der DB mit Modellen aus openclaw.json"""
    try:
        # Models aus openclaw.json laden
        model_map = {}
        try:
            with open('/home/iggy/.openclaw/openclaw.json') as f:
                cfg = json.load(f)
            for agent in cfg.get('agents', {}).get('list', []):
                model_map[agent['id']] = agent.get('model', {}).get('primary', 'unknown')
        except Exception:
        ...
        for row in rows:
            agent_data = dict(row)
            agent_data['model'] = model_map.get(row['name'], 'unknown')
            agents_with_models.append(agent_data)

        return jsonify({'agents': agents_with_models, 'count': len(agents_with_models)})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def agent_report(agent):
    ...
    valid_agents = ['zerberus', 'orpheus', 'athene', 'hestia', 'apollon', 'metamaus', 'rheingold', 'pythia', 'hermes']
    ...
    try:
        ...
        if agent == 'zerberus':
            ...
            cursor.execute("SELECT COUNT(*) as cnt FROM athene_trades WHERE timestamp::date = CURRENT_DATE")
            ...
            # Generischer Fallback-Report für alle Agenten ohne eigene Logik
            try:
                # Letzte Tasks für diesen Agent
                cursor.execute("""
                    SELECT task, status, done_at
                    FROM agent_tasks
                    WHERE agent = %s
                    ORDER BY done_at DESC NULLS LAST LIMIT 5
                """, (agent,))
                tasks = cursor.fetchall()

                # Letzte Log-Einträge für diesen Agent
                cursor.execute("""
                ...
                if tasks:
                    for t in tasks:
                        status_emoji = '✅' if t['status'] == 'done' else '⏳' if t['status'] == 'running' else '📋'
                        done = t['done_at'].strftime('%Y-%m-%d %H:%M') if t['done_at'] else '—'
                        tasks_html += f"<li>{status_emoji} {t['task']} ({t['status']})—{done}</li>"
                else:
                    tasks_html = '<li>Keine Tasks gefunden</li>'

                logs_html = ''
                if logs:
                    for log in logs:
                        level_emoji = '🔴' if log['level'] == 'ERROR' else '🟡' if log['level'] == 'WARNING' else '📝'
                        ts = log['timestamp'].strftime('%Y-%m-%d %H:%M') if log['timestamp'] else '—'
                        logs_html += f"<li>{level_emoji} [{ts}] {log['message']}</li>"
                else:
                    logs_html = '<li>Keine Logs gefunden</li>'

                report = f"""
            <h2 style="color:#a78bfa">🤖 {agent.upper()} AGENT REPORT</h2>
            <h3 style="color:#ffd700">Letzte Tasks:</h3>
            <ul style="color:#ccc">
                {tasks_html}
            </ul>
            <h3 style="color:#ffd700">Letzte Logs:</h3>
            <ul style="color:#ccc">
                {logs_html}
            </ul>
            """
            except Exception as e:
                report = f"<p>Report für {agent} konnte nicht generiert werden: {str(e)}</p>"
...
def get_knowledge():
    """Agent-Wissen aus agent_knowledge Tabelle abrufen"""
    try:
        conn = get_db_connection()
        cursor = conn.cursor(cursor_factory=RealDictCursor)
        ...
        if category:
            cursor.execute(
                """SELECT id, key, value, category, source_file, learned_at, updated_at, tags
        ...
        # Gesamtzahl ermitteln (separater cursor, kein RealDictCursor nötig)
        count_cur = conn.cursor()
        if category:
            count_cur.execute("SELECT COUNT(*) FROM agent_knowledge WHERE category = %s", (category,))
        else:
            count_cur.execute("SELECT COUNT(*) FROM agent_knowledge")
        total = count_cur.fetchone()[0]
        count_cur.close()

        # Distinct Kategorien
        count_cursor = conn.cursor()
        count_cursor.execute("SELECT DISTINCT category FROM agent_knowledge ORDER BY category")
        categories = [row[0] for row in count_cursor.fetchall()]
        count_cursor.close()

        cursor.close()
        conn.close()

        # Arrays serialisierbar machen
        result = []
        for row in rows:
            item = dict(row)
            result.append(item)
...
def serve_rheingold_content():
    """Liefert nur das BODY-Inner-HTML (kein DOCTYPE/html/head) — sicher für innerHTML."""
    ...
    try:
        with open(os.path.join('templates', 'rheingold_content.html'), 'r', encoding='utf-8') as f:
            raw = f.read()
        # Variablen ersetzen: {{ orgas_json | safe }} -> tatsächlicher JSON
        import re
        body_content = re.sub(r'\{\{\s*orgas_json\s*\|\s*safe\s*\}\}', orgas_json, raw)
        # Alles vor <body> und nach </body> abschneiden
        body_match = re.search(r'<body[^>]*>(.*)</body>', body_content, re.DOTALL | re.IGNORECASE)
        if body_match:
            body_content = body_match.group(1)
        else:
            # Fallback: head/meta/style entfernen
            body_content = re.sub(r'<style[^>]*>.*?</style>', '', body_content, flags=re.DOTALL)
            body_content = re.sub(r'<script[^>]*>.*?</script>', '', body_content, flags=re.DOTALL)
        return Response(body_content, mimetype='text/html')
    except Exception as e:
        return Response(f'<div style="color:red">Rheingold Fehler: {e}</div>', mimetype='text/html')
...
def system_health():
    try:
        ...
        try:
            ...
            pass
        now = datetime.now()
        uptime_hours = round((now - PROCESS_START_TIME).total_seconds() / 3600, 2)
        return jsonify({
...
def send_smtp_mail(to_email, subject, body):
    ...
    msg["From"] = '"[NAME]" <investigativ@abydon.com>'
    msg["To"] = to_email
    msg["Subject"] = subject
...
def rheingold_findings():
    try:
        ...
        cur = conn.cursor()
        cur.execute("""
            SELECT id, finding_type, beschreibung, betrag, quelle, created_at,
                   org_name, kategorie, foerderprogramm, jahr
        ...
        return jsonify([{
            ...
            "finding_type": r[1],
            "beschreibung": r[2],
            "betrag": float(r[3]) if r[3] else None,
            "quelle": r[4],
            "created_at": str(r[5])[:19] if r[5] else None,
            "datum": str(r[5])[:10] if r[5] else None,
            "organisation": r[6],
            "kategorie": r[7],
            "foerderprogramm": r[8],
            "jahr": r[9]
...
def rheingold_engine_status():
    ...
    try:
        ...
        # Compute dynamic activation date from earliest DB entry
        try:
            cur.execute("SELECT MIN(created_at)::date FROM rheingold_findings")
            fdate = cur.fetchone()[0]
            cur.execute("SELECT MIN(created_at)::date FROM rheingold_entities")
            edate = cur.fetchone()[0]
            import datetime
            if fdate and edate:
                earliest = min(datetime.date(fdate.year, fdate.month, fdate.day),
                               datetime.date(edate.year, edate.month, edate.day))
            elif fdate:
                earliest = fdate
            else:
                earliest = edate
            if earliest:
                status['activated_at'] = earliest.strftime('%d.%m.%Y')
        except:
            pass
        cur.close(); conn.close()
    except Exception as e:
...
def api_ifg_list():
    ...
    try:
        ...
        for req in requests:
            cur.execute("""
                SELECT id, an, betreff, status, erstellt_am, gesendet_am
...
def api_ifg_detail(ifg_id):
    ...
    try:
        ...
        # Mails
        cur.execute("""
            SELECT id, an, betreff, text_entwurf, text_final, status, erstellt_am, gesendet_am, kategorie
            FROM rheingold_mails
            WHERE ifg_request_id = %s
            ORDER BY id DESC
        """, (ifg_id,))
...
@app.route('/api/ifg/<int:ifg_id>/mail/<int:mail_id>/send', methods=['POST'])
def api_ifg_send_mail(ifg_id, mail_id):
    """POST: Mail via SMTP absenden (To, BCC, Body aus DB)"""
    try:
        import smtplib
        data = request.get_json() or {}
        user_bcc = data.get('bcc', '')
        # BCC immer ipctec@gmail.com — hardcoded, nicht änderbar
        bcc_parts = ['ipctec@gmail.com']
        if user_bcc and user_bcc.strip():
            for part in user_bcc.split(','):
                part = part.strip()
                if part and part.lower() != 'ipctec@gmail.com':
                    bcc_parts.append(part)
        bcc = ', '.join(bcc_parts)
        conn = get_db_connection()
        cur = conn.cursor()
        # Load mail from DB
        cur.execute("""
            SELECT m.an, m.betreff, COALESCE(m.text_final, m.text_entwurf) as body
            FROM rheingold_mails m
            WHERE m.id = %s AND m.ifg_request_id = %s
        """, (mail_id, ifg_id))
        row = cur.fetchone()
        if not row:
        ...
        to_email, subject, body = row
        if not body:
            return jsonify({'status': 'error', 'message': 'Mail has no body'}), 400
        # Send via SMTP with BCC
        password = get_smtp_password()
        if not password:
            return jsonify({'status': 'error', 'message': 'SMTP password not available'}), 500
        msg = EmailMessage()
        msg.set_content(body)
        msg["From"] = '"[NAME]" <investigativ@abydon.com>'
        msg["To"] = to_email
        ...
        # Update status in DB
        cur.execute("""
            UPDATE rheingold_mails SET status='gesendet', gesendet_am=NOW(),
                   text_final=COALESCE(text_final, text_entwurf)
            WHERE id = %s
        """, (mail_id,))
        # Update IFG request status
        cur.execute("""
            UPDATE rheingold_requests SET status='beschwerde_gesendet'
            WHERE id = %s AND status NOT IN ('beantwortet','beantwortet_teilweise','bescheid_erhalten')
        """, (ifg_id,))
        conn.commit()
        cur.close()
        conn.close()
        ...
        try:
            ...
            cur2 = conn2.cursor()
            import json as _json
            cur2.execute("SELECT value FROM agent_knowledge WHERE key = 'ifg7_status_20260408'")
            ak_row = cur2.fetchone()
            if ak_row:
                status_data = _json.loads(ak_row[0])
                status_data['status_db'] = 'beschwerde_gesendet_final'
                status_data['beschwerde_gesendet_am'] = now_berlin().isoformat()
                status_data['beschwerde_gesendet_an'] = to_email
                status_data['beschwerde_mail_id'] = mail_id
                status_data['timestamp'] = now_berlin().isoformat()
                cur2.execute("UPDATE agent_knowledge SET value = %s WHERE key = 'ifg7_status_20260408'",
                             (_json.dumps(status_data, ensure_ascii=False),))
                conn2.commit()
            cur2.close()
            conn2.close()
        except Exception as ak_err:
            print(f"Warning: Could not update agent_knowledge: {ak_err}")
        return jsonify({'status': 'success', 'sent_to': to_email, 'bcc': bcc, 'sent_at': now_berlin().isoformat()})
    except Exception as e:
        return jsonify({'status': 'error', 'message': str(e)}), 500


if __name__ == '__main__':
    app.run(host='127.0.0.1', port=int(os.environ.get('PORT', 5002)), debug=False, threaded=True)
